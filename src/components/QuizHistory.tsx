...
export function QuizHistory({ onBack, classId, studentName }: QuizHistoryProps) {
  ...
  const [detailOwn, setDetailOwn] = useState<QuizAnswerFullResponse | null>(null);
    ...
    async function loadAll() {
      ...
      try {
        ...
        try {
          ...
          console.warn('GET /quizzes/classes/:id/answers no OK (continúo sin romper UI):', e);
        }
      } catch (e: any) {
        if (!cancelled) {
  ...
  const openDetail = async (quizId: number | string) => {
    ...
    try {
      ...
      const quizDetail: QuizFullResponse = await quizService.getQuizInfoById(Number(quizId));
      setDetailQuiz(quizDetail);

      // 2) Tus respuestas propias a ese quiz - Backend returns data directly
      try {
        const ownAnswer: QuizAnswerFullResponse = await quizService.getOwnAnswer(Number(quizId));
        setDetailOwn(ownAnswer);
      } catch (e: any) {
        console.warn('GET /quizzes/:id/answer no OK (sin respuestas propias):', e);
      }
    } catch (e: any) {
  ...
  const completed = safeQuizzes.filter(q => ownAnswersMap[String(q.id)]?.score != null);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-accent to-muted p-4">
                    ...
                    <div className="flex items-center gap-4 flex-1">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <h3 className="font-semibold text-gray-800">Quiz {q.id}</h3>
                          {mine?.streakBonus && (
      ...
      {detailOpen && (
        ...
        >
          <Card className="w-full max-w-3xl bg-white flex flex-col shadow-xl" style={{ maxHeight: '90vh', height: '90vh' }}>
                ...
                {!detailLoading && !detailError && detailQuiz && (
                  <>
                    <div className="text-sm text-gray-600 mb-4">
                      <div>Fecha: {detailQuiz.date ? new Date(detailQuiz.date).toLocaleDateString('es-ES') : '—'}</div>
                    ...
                    <div className="space-y-3 pb-4">
                      {(detailQuiz.questions ?? []).map((q, qIndex) => {
                        const myAnswerIndex = detailOwn?.answers?.[qIndex] ?? null;
                        const correctAnswerIndex = detailOwn?.correctAnswers?.[qIndex] ?? null;
                        const options = [
                              ...
                              {options.map(o => {
                                ...
                                return (
                                  <div key={o.k} className={`p-2 border rounded ${style}`}>
                                    <span className="font-semibold mr-2">{o.label}.</span>{o.v}
                                  </div>
                                );
                              })}
                            </div>
