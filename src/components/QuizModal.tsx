...
export function QuizModal({
  ...
  useEffect(() => {
    ...
    async function loadDaily() {
      if (!token) {
        setLoadError(
          "No hay sesión activa (token). Inicia sesión para cargar el quiz.",
        );
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        try {
          ...
          const quizData: QuizFullResponse =
            await quizService.getDailyQuiz(classId);
          ...
          const quizAlreadyAnswered = allAnswers.some(
            (answer) => answer.quizId === quizData.id,
          );

          if (quizAlreadyAnswered) {
            if (!cancelled) {
              setLoadError(
                "Ya has completado el quiz diario de hoy. Vuelve mañana para un nuevo quiz.",
              );
            }
            return;
          }
          ...
          if (!quizData || !Array.isArray(quizData.questions)) {
            throw new Error(
              "Respuesta inesperada del backend: faltan preguntas.",
            );
          }
  ...
  const handleFinishQuiz = async () => {
    ...
    try {
      ...
      const response: AnswerDailyQuizResponse =
        await quizService.answerDailyQuiz(classId, {
      ...
      if (response.score != null && response.total != null) {
        ...
        finalScore = Math.round(
          (correctCount / Math.max(1, answersArray.length)) * 100,
        );
      }

      // Usar las monedas que vienen del backend
      finalCoins = response.coinsEarned ?? 0;
    } catch (e: any) {
      // Verificar si el error es porque ya se respondió el quiz
      if (e?.response?.status === 400 || e?.response?.status === 409) {
        setSubmitError(
          "Ya has completado el quiz diario de hoy. No puedes responderlo dos veces.",
        );
      ...
      finalCoins = 0;
      setSubmitError(
        `No se pudo comunicar con el servidor: ${e?.message || "Error desconocido"}`,
      );
        ...
        <Card className="w-full max-w-2xl bg-white">
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-red-600">
              No hay token disponible. Inicia sesión para acceder al quiz.
            </p>
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
        <Card className="w-full max-w-2xl bg-white">
          <CardHeader>
            <CardTitle>Cargando quiz diario…</CardTitle>
          </CardHeader>
  ...
  if (loadError || questions.length === 0) {
    return (
      <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
        <Card className="w-full max-w-2xl bg-white">
          <CardHeader>
            <CardTitle>{loadError ? "Error" : "Sin preguntas hoy"}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {loadError ? (
              <p className="text-sm text-red-600">{loadError}</p>
            ) : (
              <p className="text-sm">Vuelve más tarde.</p>
            )}
            <div className="flex justify-end">
              <Button variant="outline" onClick={onClose}>
                Cerrar
      ...
      <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
        <Card className="w-full max-w-md bg-white">
          ...
          <CardContent className="text-center space-y-6">
            ...
            <div className="bg-gradient-to-br from-yellow-50 to-amber-50 p-8 rounded-lg border-2 border-yellow-200">
              <p className="text-sm text-muted-foreground mb-2">Has ganado</p>
              <p className="text-5xl font-bold text-yellow-600 mb-1">
                {coinsEarned}
              </p>
              <p className="text-lg font-medium text-yellow-700">KumoSoles</p>
            </div>
            <Button
              onClick={() => {
                onComplete(score);
        ...
        <CardHeader>
          ...
          {streakMultiplier > 1 && (
            <Badge variant="secondary" className="w-fit">
              Bonus de racha: x{streakMultiplier} KumoSoles
            </Badge>
          )}
        </CardHeader>
